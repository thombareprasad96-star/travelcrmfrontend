...
export default function CreateReminder() {
  ...
  leadService
    ...
    .then((res) => {
      const body = res.data;
      const rows = body?.data?.content ?? body?.data ?? body?.content ?? body ?? [];
      setLeads(Array.isArray(rows)
        ? rows.map(lead => ({ ...lead, id: lead.publicId || lead.id }))
        : []);
    })
    .catch(() => {
      showToast("Failed to load leads.", "error");
  ...
  const handleLeadChange = useCallback((e) => {
    const selectedId = e.target.value;            // lead.id is the UUID (publicId)
    const lead = leads.find((l) => String(l.id) === String(selectedId));
    setValue("phone", lead?.phone || "");
  }, [leads, setValue]);
  ...
  const onSubmit = async (data) => {
    setSubmitting(true);
    try {
      const selectedLead = leads.find(l => String(l.id) === String(data.leadId));
      const payload = {
        title:       data.title.trim(),
        description: data.description.trim(),
        type:        data.type,
  ...
  /* ── Live summary values ── */
  const selectedLead     = leads.find(l => String(l.id) === String(watchAll.leadId));
  const selectedUser     = users.find(u => String(u.publicId || u.id) === String(watchAll.assignTo));
  const selectedType     = REMINDER_TYPES.find(t => t.value === watchAll.type);
  const selectedPriority = PRIORITIES.find(p => p.value === watchAll.priority);
  const dueDateDisplay   = watchAll.dueDate
    ? new Date(watchAll.dueDate).toLocaleString("en-IN", {
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="">Select User</option>
                      {users.map(u => (
                        <option key={u.publicId || u.id} value={u.publicId || u.id}>{u.fullName || u.name || u.username}</option>
                      ))}
                    </select>
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-[10px] pointer-events-none">▼</span>
                  </div>
