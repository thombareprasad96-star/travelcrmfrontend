// src/features/marketing/components/marketingUi.jsx
// ─────────────────────────────────────────────────────────────────────────────
// Shared UI kit for the Marketing & Campaigns section. Same visual language as the
// Reports / Bookings / Accounting modules (gradient page header + breadcrumb,
// gradient hero stat cards with count-up, glass panels, fadeUp/popIn animations,
// styled tables, gradient-header modals, filter pills, pager) — blue-600→indigo-500
// primary with a gold (#eeda92) accent, matching the report/booking/customer palette.
// ─────────────────────────────────────────────────────────────────────────────
import { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";
import { ChevronDown, X, Inbox, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight, MessageCircle, Mail } from "lucide-react";
...
const GLOBAL_STYLE = `
  ...
  .mkt-fade { animation:fadeUp .4s ease both; }
  .mkt-scope select { -webkit-appearance:none; appearance:none; }
  .mkt-scope ::-webkit-scrollbar{width:5px;height:5px}
  .mkt-scope ::-webkit-scrollbar-track{background:#f1f5f9;border-radius:99px}
  .mkt-scope ::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:99px}
`;
...
export function Page({ icon: Icon, title, subtitle, crumb, actions, children }) {
  ...
  return (
    <div className="mkt-scope min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-100"
        ...
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-500 flex items-center justify-center text-white shadow-lg shadow-blue-200 flex-shrink-0">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 hidden sm:block">
                  ...
                  <span className="hover:text-blue-600 cursor-pointer transition-colors" onClick={() => navigate("/marketing/campaigns")}>Marketing</span>
                  {crumb && <><span className="mx-1 text-slate-300">/</span><span className="text-blue-600 font-bold">{crumb}</span></>}
...
/* ── Section tabs (Segments / Campaigns / Drips / Automations) ─────────────────── */
...
export function Hero({ label, value, sub, gradient = "from-blue-600 to-indigo-500", icon, delay = 0, isString = false }) {
  ...
  const shown = isString ? value : Math.round(disp).toLocaleString("en-IN");
  return (
    <div className={`bg-gradient-to-br ${gradient} rounded-2xl p-5 text-white shadow-lg relative overflow-hidden group hover:-translate-y-1 hover:shadow-xl transition-all duration-300 mkt-fade`}
...
export function Panel({ className = "", children, style }) {
  return (
    <div className={`bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/60 shadow-sm mkt-fade ${className}`} style={style}>
      {children}
    </div>
  );
}

export function PanelHead({ icon: Icon, iconColor = "text-blue-500", title, count, countColor = "bg-blue-600", right }) {
  return (
...
/** Channel chip (WhatsApp / Email) with matching icon + tone. */
export function ChannelBadge({ channel }) {
  const wa = channel === "WHATSAPP";
  return (
    <Badge tone={wa ? "green" : "blue"}>
      {wa ? <MessageCircle className="w-3 h-3" /> : <Mail className="w-3 h-3" />}
      {wa ? "WhatsApp" : "Email"}
    </Badge>
  );
}
...
/* ── Button ────────────────────────────────────────────────────────────────────── */
const BTN_VARIANTS = {
  primary: "bg-gradient-to-r from-blue-600 to-indigo-500 hover:from-blue-700 hover:to-indigo-600 text-white shadow-md shadow-blue-200",
  ...
  gold: "bg-gradient-to-r from-gold-400 to-gold-600 hover:from-gold-500 hover:to-gold-700 text-white shadow-md shadow-gold-200",
  outline: "border border-slate-200 hover:border-blue-300 bg-white hover:bg-blue-50 text-slate-600 hover:text-blue-600 shadow-sm",
...
export function IconBtn({ tone = "blue", title, onClick, disabled, children }) {
  const tones = {
  ...
  return (
    <button type="button" title={title} onClick={onClick} disabled={disabled}
      className={`w-8 h-8 rounded-lg border flex items-center justify-center transition-all disabled:opacity-40 disabled:pointer-events-none ${tones[tone] || tones.blue}`}>
      {children}
    </button>
  );
}
...
/* ── Form primitives ──────────────────────────────────────────────────────────── */
export const inputCls = "w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 font-medium placeholder-slate-400 focus:border-blue-400 focus:ring-2 focus:ring-blue-50 outline-none transition-all hover:border-slate-300 disabled:bg-slate-50 disabled:text-slate-400";
...
export function Toggle({ label, hint, checked, disabled, onChange }) {
  return (
    <label className={`flex items-start gap-2.5 rounded-xl border px-3.5 py-2.5 transition-all ${disabled ? "opacity-60 border-slate-200" : "cursor-pointer border-slate-200 hover:border-blue-300"} ${checked ? "bg-blue-50/60 border-blue-200" : "bg-white"}`}>
...
export function Modal({ title, subtitle, icon: Icon, onClose, children, footer, maxW = "max-w-xl", gradient = "from-blue-600 to-indigo-500" }) {
  return (
...
export function Th({ children, right, className = "" }) {
  return <th className={`px-3 py-3.5 text-[11px] font-extrabold text-slate-500 uppercase tracking-wider whitespace-nowrap ${right ? "text-right" : "text-left"} ${className}`}>{children}</th>;
}

/* ── AllLeads-style grid list (header bar + gold-hover rows, no expandable) ──────── */
// Mirrors the Leads Directory: a CSS-grid header bar and rows with column separators,
// a soft gold (#eeda92) row hover, and a staggered fadeUp. `cols` is a CSS
// grid-template-columns string (e.g. "1.6fr 0.95fr 124px").

export function GridHead({ cols, children }) {
  return (
    <div className="hidden md:grid items-stretch gap-0 px-5 py-3 bg-slate-50/80 text-[11px] font-extrabold text-slate-400 uppercase tracking-wider border-b border-slate-100"
...
export function GridRow({ cols, index = 0, children, className = "" }) {
  return (
    <div className={`hidden md:grid items-stretch gap-0 px-5 py-3.5 border-b border-slate-50 transition-colors group ${className}`}
      style={{ gridTemplateColumns: cols, animation: "fadeUp .35s ease both", animationDelay: `${index * 30}ms` }}
...
/** Grid cell. `first` = left-aligned, no left separator; otherwise centered (or `right`) with a left separator. */
...
/** Circular gradient avatar showing the first initial (matches the Leads Directory). */
export function Avatar({ text, gradient = "from-blue-500 to-indigo-600", size = "w-10 h-10", className = "" }) {
  const initial = String(text || "?").trim().charAt(0).toUpperCase() || "?";
  return <div className={`${size} rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-white text-sm font-extrabold shadow-sm flex-shrink-0 ${className}`}>{initial}</div>;
}
...
/** Centered empty state for the grid list (matches the Leads Directory empty block). */
export function GridEmpty({ icon: Icon = Inbox, title = "Nothing here yet", hint, action }) {
  return (
    <div className="text-center py-20 px-5">
      <div className="flex flex-col items-center justify-center">
        <div className="w-16 h-16 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center mb-4 shadow-sm -rotate-3"><Icon size={28} className="text-slate-400" /></div>
...
export function Loading({ label = "Loading…" }) {
  return (
    <div className="flex flex-col items-center justify-center py-20">
      <div className="w-10 h-10 rounded-full border-[3px] border-blue-100 border-t-blue-600 animate-spin mb-3" />
...
/* ── Pager (reports style; 0-based page) ──────────────────────────────────────── */
      ...
      <div className="flex items-center gap-1.5">
        ...
        {pages.map((p, i) => typeof p === "string"
          ...
          : <button key={p} onClick={() => onPage(p)}
              className={`w-8 h-8 rounded-lg text-xs font-bold border transition-all ${page === p ? "bg-gradient-to-r from-blue-600 to-indigo-500 border-blue-600 text-white shadow-sm" : "bg-white border-slate-200 text-slate-600 hover:border-blue-300 hover:text-blue-600"}`}>{p + 1}</button>)}
...
/* ── Status badge helpers (shared tone maps) ──────────────────────────────────── */
export const CAMPAIGN_STATUS_TONE = {
  DRAFT: "slate", SCHEDULED: "amber", SENDING: "blue", SENT: "green", FAILED: "red", CANCELLED: "rose",
};
export const DRIP_STATUS_TONE = { DRAFT: "slate", ACTIVE: "green", PAUSED: "amber" };
export const RECIPIENT_STATUS_TONE = { PENDING: "slate", SENT: "green", FAILED: "red", SKIPPED: "amber" };
export const ENROLLMENT_STATUS_TONE = { ACTIVE: "green", COMPLETED: "blue", CANCELLED: "slate" };
