...
function normalizeBooking(b = {}) {
  ...
  // FIX: Expanded key checks to catch whatever your backend is actually sending
  const adults = Number(b.adults || b.numAdults || b.noOfAdults || b.adultCount || 0);
  const children = Number(b.children || b.numChildren || b.noOfChildren || b.childCount || 0);
  const infants = Number(b.infants || b.numInfants || b.noOfInfants || b.infantCount || 0);
...
export default function BookingDetails() {
  ...
  const fetchBooking = useCallback(async () => {
    ...
    try {
      ...
      // TIP: If travelers still aren't showing, uncomment this line and check the console 
      // to see the exact field names coming from your API!
      // console.log("API BOOKING RESPONSE:", unwrap(res));
  ...
  // FIX: Forced Number parsing to ensure it correctly totals integers instead of concatenating strings
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {b.children > 0 && <p className="text-sm text-slate-700"><span className="font-semibold">Children:</span> {b.children}</p>}
                    {b.infants  > 0 && <p className="text-sm text-slate-700"><span className="font-semibold">Infants:</span> {b.infants}</p>}
                    <p className="text-sm font-bold text-slate-700">Total: {totalTravellers > 0 ? totalTravellers : "—"}</p>
                  </div>
                </div>
              </div>
            </SCard>
