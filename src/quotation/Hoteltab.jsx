...
//    MAIN HOTEL TAB
// ═══════════════════════════════════════════════════════ */
// export default function HotelTab({ onDataChange, paxInfo = {} }) {
//   const [included, setIncluded] = useState(true);
//   const [title,    setTitle]    = useState("Hotel Details");
//   const [notes,    setNotes]    = useState("");
//   const [hotels,   setHotels]   = useState([newHotel()]);
...
//   // ── Flexible city match: "Dubai" ↔ "Dubai, UAE" ↔ "dubai city" sab match ──
//   const cityMatches = (h) => {
...
//       })
//     : cityFilterActive
//       ? (allHotels || []).filter(cityMatches)
//       : (allHotels || []);

//   // DEBUG — city match kyun fail ho raha, dekho
//   if (cityFilterActive && (allHotels || []).length > 0) {
//     console.log(`[HOTEL city-filter] looking for="${defaultCity}" | matched=${filtered.length}/${allHotels.length}`,
//       allHotels.slice(0, 5).map(h => getCity(h)));
//   }
...
//             <MapPin size={11} /> Showing {defaultCity} hotels
//           </span>
...
//         {filtered.length === 0 ? (
//           <li className="px-3 py-4 text-sm text-slate-400 text-center">
//             {(allHotels || []).length === 0 ? "No hotels found in master" : cityFilterActive ? `No hotels found for ${defaultCity}` : "No matching hotels"}
//           </li>
//         ) : (
//           filtered.map((h, idx) => {
//             const img  = getImage(h);
