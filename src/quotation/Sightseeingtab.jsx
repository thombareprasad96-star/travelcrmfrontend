...
// ═══════════════════════════════════════════════════════ */
// function SightseeingSearchDropdown({ value, onSelect, allItems, loading, placeholder = "Type to search attractions..." }) {
...
//         const city  = getCity(s).toLowerCase();
...
//             {(allItems || []).length === 0 ? "No attractions in master" : "No matching attractions"}
//           </li>
//         ) : (
//           filtered.map((s, idx) => {
...
//           <button type="button" onClick={onClose} disabled={saving} className="px-5 py-2.5 rounded-xl border border-slate-300 font-bold text-slate-700 hover:bg-slate-50 transition text-sm">Cancel</button>
//           <button type="button" onClick={handleSave} disabled={saving || imgUploading} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-xl font-bold text-sm shadow-md transition active:scale-95 flex items-center gap-2">
...
//                           placeholder="Type to search attractions..."
//                         />
//                       </div>
//                       {/* ➕ Add new sightseeing */}
//                       <button type="button" onClick={() => openAddModal(day.id, act.id)} title="Add new attraction"
//                         className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-xl border border-blue-500 text-blue-600 hover:bg-blue-600 hover:text-white transition shadow-sm">
...
//                         className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-xl border border-slate-300 text-slate-600 hover:bg-slate-100 transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed">
//                         <Pencil size={15} />
//                       </button>
//                     </div>
//                   </div>
...
//   const [open,   setOpen]   = useState(false);
//   const [search, setSearch] = useState("");
//   const [showAllCities, setShowAllCities] = useState(false);  // city-filter override
//   const [pos,    setPos]    = useState(null);
...
//   const term = search.trim().toLowerCase();
//   const cityFilterActive = !!defaultCity && !showAllCities && !term;
...
//       })
//     : cityFilterActive
//       ? (allItems || []).filter(cityMatches)
//       : (allItems || []);

//   // DEBUG — city match kyun fail ho raha
//   if (cityFilterActive && (allItems || []).length > 0) {
//     console.log(`[SIGHTSEEING city-filter] looking for="${defaultCity}" | matched=${filtered.length}/${allItems.length}`,
//       allItems.slice(0, 5).map(s => getCity(s)));
//   }
...
//             <MapPin size={11} /> Showing {defaultCity} attractions
//           </span>
...
//         {filtered.length === 0 ? (
//           <li className="px-3 py-4 text-sm text-slate-400 text-center">
//             {(allItems || []).length === 0 ? "No attractions in master" : cityFilterActive ? `No attractions found for ${defaultCity}` : "No matching attractions"}
//           </li>
//         ) : (
//           filtered.map((s, idx) => {
//             const img  = getImage(s);
...
//       {included && days.map((day, di) => (
//         <SectionCard key={day.id} title={day.city ? `Day ${day.day} — ${day.city}` : `Day ${day.day}`} icon={Sun} iconColor="text-amber-600"
//           headerRight={
//             <div className="flex items-center gap-2">
//               <Input type="date" value={day.date} onChange={e => updateDay(day.id, "date", e.target.value)} className="w-36 text-xs py-1.5" />
//               {days.length > 1 && <RemoveBtn onClick={() => removeDay(day.id)} />}
