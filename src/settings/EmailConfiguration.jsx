// src/components/Settings/EmailConfiguration.jsx
// ─────────────────────────────────────────────────────────────
// Email Configuration Page — Travel CRM
// Route: /Settings/Email
// Matches CRM design system: glass cards, gradients, Plus Jakarta Sans
// Reference: company/settings/email/index.php screenshot
// Features:
//   - SMTP Configuration form (left panel, large)
//   - Common SMTP Settings sidebar (right panel)
//   - Fields: SMTP Host | SMTP Port | Encryption | Username | Password | From Email | From Name
//   - Show/Hide password toggle
//   - Save Configuration + Test Email + Back to Settings buttons
//   - Test Email modal (send test to any address + show result)
...
function TestEmailModal({ onClose, onSend }) {
  ...
  const handleSend = async () => {
    ...
    try {
      // BACKEND: await emailSettingsService.testEmail(testEmail);
      await new Promise(r => setTimeout(r, 1800));
      setResult("success");
      setResultMsg(`Test email successfully sent to ${testEmail}`);
    } catch {
      setResult("error");
      setResultMsg("Failed to send test email. Check your SMTP configuration.");
    } finally {
      setSending(false);
    }
  };
...
export default function EmailConfiguration() {
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      // BACKEND: await emailSettingsService.saveConfig(form);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-100"
      ...
      {showTestModal && (
        <TestEmailModal onClose={() => setTestModal(false)} onSend={() => setTestModal(false)}/>
      )}
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden fade-up">
              ...
              <div className="bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-500 px-6 py-4 flex items-center gap-3">
                ...
                <div className="ml-auto flex items-center gap-2 bg-white/20 px-3 py-1.5 rounded-full">
                  ...
                  <span className="text-white/80 text-xs font-bold">Not Configured</span>
                </div>
              </div>
