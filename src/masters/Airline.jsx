...
// const dummyAirlines = [
//   { id: 1, name: "Air India", status: "Active", createdAt: "2023-01-15", logo: null, initials: "AI", color: "from-red-500 to-red-700", iata: "AI" },
//   { id: 2, name: "IndiGo", status: "Active", createdAt: "2023-03-22", logo: null, initials: "6E", color: "from-indigo-500 to-indigo-700", iata: "6E" },
//   { id: 3, name: "SpiceJet", status: "Active", createdAt: "2023-05-10", logo: null, initials: "SJ", color: "from-orange-500 to-red-500", iata: "SG" },
//   { id: 4, name: "Vistara", status: "Inactive", createdAt: "2023-07-01", logo: null, initials: "UK", color: "from-purple-600 to-purple-800", iata: "UK" },
//   { id: 5, name: "Emirates", status: "Active", createdAt: "2023-08-18", logo: null, initials: "EK", color: "from-red-600 to-yellow-600", iata: "EK" },
//   { id: 6, name: "Qatar Airways", status: "Active", createdAt: "2023-09-05", logo: null, initials: "QR", color: "from-rose-700 to-rose-900", iata: "QR" },
//   { id: 7, name: "Singapore Airlines", status: "Active", createdAt: "2023-11-20", logo: null, initials: "SQ", color: "from-blue-600 to-cyan-600", iata: "SQ" },
// ];
...
//   if (airline.logo) return <img src={airline.logo} alt={airline.name} className={`${sz} rounded-xl object-contain bg-white border border-gray-100 shadow-sm`} />;
//   return (
//     <div className={`${sz} rounded-xl bg-gradient-to-br ${airline.color || "from-blue-500 to-blue-700"} flex items-center justify-center shadow-sm flex-shrink-0`}>
//       <span className="font-bold text-white leading-none">{airline.initials}</span>
//     </div>
//   );
// }
...
//   return (
//     <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_STYLES[status] || STATUS_STYLES.Inactive}`}>
//       {icons[status]}{status}
//     </span>
//   );
// }
...
//             <h2 className="text-base font-bold text-gray-900">Airline Details</h2>
...
//                 <p className="text-sm text-gray-500 mt-0.5">IATA: {airline.iata}</p>
//                 <div className="mt-2"><StatusBadge status={airline.status} /></div>
//               </div>
//             </div>
//             <div className="grid grid-cols-2 gap-4">
//               {[["Airline ID", `#${String(airline.id).padStart(4, "0")}`], ["Date Added", formatDate(airline.createdAt)], ["Status", airline.status], ["Logo", airline.logo ? "Uploaded" : "Auto-generated"]].map(([k, v]) => (
...
//             <button onClick={onClose} className="px-5 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition">Close</button>
//           </div>
//         </>
//       )}
//     </Modal>
...
//   const [form, setForm] = useState({ name: "", status: "Active", logo: null, preview: null });
//   const [errors, setErrors] = useState({});
//   const [drag, setDrag] = useState(false);
//   const fileRef = useRef();

//   const filtered = useMemo(() => {
//     let list = airlines.filter(a => a.name.toLowerCase().includes(search.toLowerCase()));
//     list = [...list].sort((a, b) => {
...
//   const openAdd = () => { setForm({ name: "", status: "Active", logo: null, preview: null }); setErrors({}); setEditing(null); setShowAdd(true); };
//   const openEdit = (a) => { setForm({ name: a.name, status: a.status, logo: a.logo, preview: a.logo }); setErrors({}); setEditing(a); setShowAdd(true); };
...
//     if (editing) {
//       setAirlines(prev => prev.map(a => a.id === editing.id ? { ...a, name: form.name, status: form.status, logo: form.logo || a.logo } : a));
//     } else {
//       const initials = form.name.split(" ").map(w => w[0]).slice(0, 2).join("").toUpperCase();
//       setAirlines(prev => [...prev, { id: Date.now(), name: form.name, status: form.status, logo: form.logo, initials, color: AIRLINE_COLORS[prev.length % AIRLINE_COLORS.length], createdAt: new Date().toISOString().slice(0, 10), iata: initials }]);
//     }
//     setSaving(false);
//     setShowAdd(false);
//   };
...
//               <div className="w-10 h-10 bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl flex items-center justify-center shadow-md shadow-blue-100">
//                 <Plane size={18} className="text-white" />
//               </div>
//               <div>
//                 <h1 className="text-xl font-bold text-gray-900 leading-none">Airline Master</h1>
...
//                 <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search airlines..." className="border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-56 transition bg-slate-50 focus:bg-white" />
//               </div>
//               <button onClick={openAdd} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md shadow-blue-200 whitespace-nowrap">
...
//           {[
//             { label: "Total Airlines", value: airlines.length, color: "text-blue-600 bg-blue-50" },
//             { label: "Active", value: activeCount, color: "text-green-600 bg-green-50" },
//             { label: "Inactive", value: airlines.filter(a => a.status === "Inactive").length, color: "text-gray-600 bg-gray-100" },
//             { label: "Suspended", value: airlines.filter(a => a.status === "Suspended").length, color: "text-red-600 bg-red-50" },
//           ].map(s => (
...
//             <div>
//               <h2 className="text-sm font-bold text-gray-800">Airlines List</h2>
...
//               {!search && (
//                 <button onClick={openAdd} className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition">
...
//                     <th className="text-left px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-16">Logo</th>
//                     <th className="text-left px-4 py-3.5 cursor-pointer select-none" onClick={() => handleSort("name")}>
//                       <span className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wide hover:text-gray-800 transition">
//                         Airline Name <SortIcon field="name" />
//                       </span>
//                     </th>
//                     <th className="text-left px-4 py-3.5 cursor-pointer select-none hidden sm:table-cell" onClick={() => handleSort("createdAt")}>
//                       <span className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wide hover:text-gray-800 transition">
//                         Created Date <SortIcon field="createdAt" />
//                       </span>
//                     </th>
//                     <th className="text-left px-4 py-3.5 cursor-pointer select-none" onClick={() => handleSort("status")}>
//                       <span className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wide hover:text-gray-800 transition">
...
//                     <tr key={airline.id} className="hover:bg-blue-50/30 transition-colors group"
...
//                         <div>
//                           <p className="font-semibold text-gray-800 text-sm group-hover:text-blue-700 transition">{airline.name}</p>
//                           <p className="text-xs text-gray-400 mt-0.5">IATA: {airline.iata}</p>
//                         </div>
//                       </td>
//                       <td className="px-4 py-4 hidden sm:table-cell">
//                         <span className="text-sm text-gray-500">{formatDate(airline.createdAt)}</span>
//                       </td>
//                       <td className="px-4 py-4">
...
//                           <button onClick={() => setViewing(airline)} title="View" className="p-2 rounded-xl hover:bg-blue-50 text-gray-400 hover:text-blue-600 transition active:scale-90">
//                             <Eye size={15} />
//                           </button>
//                           <button onClick={() => openEdit(airline)} title="Edit" className="p-2 rounded-xl hover:bg-amber-50 text-gray-400 hover:text-amber-600 transition active:scale-90">
//                             <Edit2 size={15} />
//                           </button>
//                           <button onClick={() => setDeleting(airline)} title="Delete" className="p-2 rounded-xl hover:bg-red-50 text-gray-400 hover:text-red-500 transition active:scale-90">
//                             <Trash2 size={15} />
//                           </button>
//                         </div>
//                       </td>
...
//                   className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition ${form.status === s ? s === "Active" ? "bg-green-600 border-green-600 text-white" : s === "Suspended" ? "bg-red-500 border-red-500 text-white" : "bg-gray-600 border-gray-600 text-white" : "border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
//                   {s}
//                 </button>
//               ))}
//             </div>
...
//                 <p className="text-xs text-green-600 font-medium flex items-center gap-1"><CheckCircle2 size={13} />Logo uploaded successfully</p>
//                 <button onClick={() => setForm(f => ({ ...f, logo: null, preview: null }))} className="absolute top-3 right-3 p-1 rounded-lg hover:bg-white text-gray-400 transition">
//                   <X size={14} />
//                 </button>
//               </div>
//             ) : (
...
//           <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition active:scale-95 shadow-md shadow-blue-200">
...
//               <p className="text-sm text-gray-500">Are you sure you want to delete <span className="font-semibold text-gray-800">{deleting.name}</span>? This action cannot be undone.</p>
//             </div>
//             <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end">
