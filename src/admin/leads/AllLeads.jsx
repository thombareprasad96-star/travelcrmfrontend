...
import { leadService } from '../../services/leadService'; // Adjust path if needed
import {
  Users, Trophy, PieChart, TrendingUp, Search, Flame, Snowflake,
  DownloadCloud, FileText, Plus, ChevronUp, ChevronRight, ChevronLeft,
  Inbox, User, ArrowUpRight, ArrowUpDown, ArrowUp, ArrowDown, Calendar, ChevronDown,
...
/* ─── PAGINATION HELPER FUNCTIONS & COMPONENTS ────── */
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              [User, 'Assigned To', lead.assignTo || 'Unassigned', 'bg-orange-50 text-orange-600'],
              [Calendar, 'Created', lead.createdAt ? new Date(lead.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—', 'bg-teal-50 text-teal-600'],
          ...
          {lead.itinerary && lead.itinerary.length > 0 && (
            <div>
              <p className="text-sm font-extrabold text-slate-700 mb-3 flex items-center gap-2">
                <MapPin size={14} className="text-blue-500" /> Destination & Itinerary
              </p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {lead.services.map((service, i) => (
                  <span key={i} className="bg-blue-50 text-blue-700 border border-blue-100 px-2.5 py-1 rounded-full text-[10px] uppercase font-bold tracking-wider">
                    {service}
                  </span>
                ))}
              </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto z-10">
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-500 px-6 py-5 rounded-t-2xl flex items-center justify-between">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 text-white flex items-center justify-center transition-all">
            <X size={16} />
          </button>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={() => onSave(form)}
              className="flex-1 py-2.5 rounded-xl text-white font-bold text-sm transition-all shadow-md bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200">
              Save Changes
            </button>
          </div>
        </div>
...
const Leads = () => {
  ...
  const handleUpdateSubmit = async (updatedFormData) => {
    try {
      const leadPublicId = editLead.publicId || editLead.id; 
      const services = editLead.services || [];
      const itinerary = editLead.itinerary || [];

      const completeFormData = { ...editLead, ...updatedFormData };

      await leadService.updateLead(leadPublicId, completeFormData, services, itinerary);
      
      showToast("Lead updated successfully!");
      
      setLeads(prevLeads => prevLeads.map(lead => 
        lead.id === editLead.id ? { ...lead, ...updatedFormData } : lead
      ));
    ...
    >
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-3 flex-wrap items-stretch sm:items-center">
              ...
              <div className="relative min-w-[160px]">
                <select
                  value={dateFilter}
                  onChange={(e) => setDateFilter(e.target.value)}
                  className="w-full pl-9 pr-8 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-600 font-medium
              ...
              {dateFilter === 'custom' && (
                <div className="flex items-center gap-2 fade-up">
                  <input
                    ...
                    value={startDate}
                    onChange={(e) => setStartDate(e.target.value)}
                    className="px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-600
                      focus:border-blue-400 focus:ring-2 focus:ring-blue-50 outline-none transition-all"
                  />
                  <span className="text-slate-400 text-sm font-medium">to</span>
                  <input
              ...
              {/* User Dropdown */}
              <div className="relative min-w-[150px]">
                <select className="w-full pl-9 pr-8 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-600 font-medium
                  focus:border-blue-400 focus:ring-2 focus:ring-blue-50 outline-none appearance-none cursor-pointer transition-all">
                  <option>All Assignees</option>
                </select>
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
          ...
          <div className="px-5 py-4 border-b border-slate-100 overflow-x-auto">
            <div className="flex gap-2 min-w-max">
              <button className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-md shadow-blue-200">
                All <span className="bg-white/20 px-2 py-0.5 rounded-md text-xs">{totalElements}</span>
              </button>
              <button className="bg-white text-slate-600 border border-slate-200 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:border-blue-300 hover:text-blue-600 transition-all shadow-sm">
                <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-sm shadow-emerald-500/50"></div> Fresh
              </button>
              <button className="bg-white text-slate-600 border border-slate-200 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:border-blue-300 hover:text-blue-600 transition-all shadow-sm">
                New Lead <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded-md text-xs font-black">0</span>
            ...
            <table className="w-full min-w-[1400px] border-collapse text-sm whitespace-nowrap">
              <thead className="bg-slate-50/80 border-b border-slate-100">
                <tr className="text-slate-500 text-xs uppercase tracking-wider font-extrabold">
                  ...
                  <th
                    onClick={toggleDateSort}
                    className="px-4 py-3.5 text-left cursor-pointer hover:text-blue-600 select-none transition-colors"
                  >
                    <div className="flex items-center gap-1.5">
              ...
              <tbody className="divide-y divide-slate-50 bg-white">
                {loading ? (
                    ...
                    <td colSpan="17" className="text-center py-24">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-sm text-slate-400 mb-5 max-w-sm mx-auto leading-relaxed">
                          We couldn't find any leads matching your selected criteria.
                        </p>
                        <button
                          onClick={() => { setDateFilter('all'); setSearchTerm(''); }}
                  ...
                  // 👉 MAPPING OVER currentLeads INSTEAD OF sortedLeads
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-center font-medium text-slate-700">
                        {lead.itinerary && lead.itinerary.length > 0 ? (
                          <div className="flex flex-col gap-1 items-center">
                            {lead.itinerary.map((item, i) => (
                              <span key={i} className="bg-slate-100 px-2 py-1 rounded-lg text-xs font-semibold text-slate-600">
                                {item.destination} ({item.nights}N)
                              </span>
                            ))}
                          </div>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex flex-wrap gap-1.5 w-64">
                          {lead.services && lead.services.map((service, i) => (
                            <span key={i} className="bg-blue-50 text-blue-700 border border-blue-100 px-2 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wider">
                              {service}
                            </span>
                          ))}
                        </div>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold text-slate-700">{lead.assignTo || 'Unassigned'}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5 text-sm font-extrabold text-slate-800">N/A</td>
                      <td className="px-4 py-3.5 text-slate-400">—</td>
                      <td className="px-4 py-3.5">
                        <span className="text-xs font-bold px-2.5 py-1 rounded-full border border-slate-200 bg-slate-100 text-slate-700">
                          {lead.leadType || 'N/A'}
                        </span>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        ...
                        <p className="text-xs text-slate-400">
                          {lead.createdAt ? new Date(lead.createdAt).toLocaleTimeString('en-US', {
                            hour: '2-digit', minute: '2-digit'
                          }) : ''}
                        </p>
                      </td>
